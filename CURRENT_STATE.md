...
- **Shape infill (2026-09):** `shape_infill.py` + "Infill a shape with perforations" UI section. Upload **DXF / SVG / AI-PDF** outlines (letters, logos, multi-graphic files, interior voids via even-odd) and fill with the straight/staggered hole grid (`_hole_centers` math, same margin semantics). Pure numpy containment (no shapely/GEOS): a hole is kept when its center is inside and ≥ `margin + hole_r` from every boundary segment. **Letter-aware fill** (default; `spacing_flex` > 0, UI "Spacing flex %"), two engines split by local stroke width: (1) **Stroke-band fill** (`stroke_band_fill`) owns letter-width zones — medial chains are traced per stroke (boundary chord-casting; stable-width runs bridged straight through junctions when the same width resumes, so a stem passes a diagonal whole while the diagonal's rake into the stem is trimmed), then each chain gets rows-across derived from ITS width (equal strokes ⇒ identical fill), rows anchored to the traced wall at exact clearance (junction-proof straight columns), holes justified along at ~pitch with alternate rows half-phased (staggered read), open-chain rows extended straight past chain ends until blocked (diagonal rows stop where stem rows begin), closed loops (o counters) divide evenly per row. Chains ≥2 rows report as fill; 1-row chains are the centerline (`midline` count, margin may relax when narrow_fill). (2) **Elastic lattice** (`elastic_lattice_fill`) fills wider zones: one lattice, exact stagger angle, global scale (±flex) + phase sweep, local relaxation (boundary projection + crowding repulsion, tether ≤ flex×pitch); degenerates to the rigid grid on friendly shapes (0 moved). Bare sharp corners get an anchor hole post-pass. **Perimeter outline row** (now default OFF — best for wide-stroke graphics; caused a "racetrack" look on letter-width strokes) traces every outline with corner-anchored holes at exact clearance, spacing justified per edge; fill holes crowding the ring (< 0.8×pitch) are excluded. Classic path (`spacing_flex=0`): **auto-align grid** phase sweep + **edge nudge** (capped inward slide, same Ø, min 1.05×Ø separation). **Stroke-width intelligence:** `analyze_strokes` measures stroke widths by chord-casting from the boundary and reports rows-across plus the pitch (or artwork scale, or a smaller hole Ø at pitch 2×Ø via `fit_dia`) needed for 3 rows; **auto-fit pitch** (`auto_pitch`, UI default ON, `target_rows` 2–6 default 3) makes the adjustment itself: tightens the working pitch (never loosens; floor `MIN_PITCH_FACTOR`=1.2×Ø) so `target_rows` rows span the 25th-pct stroke-regime width — full infill that reflects the letter — and reports the pitch used via `InfillResult.pitch_used`. **Fill narrow strokes** (default on) chains a centerline row through zones too narrow for grid rows between the perimeter rings (edge margin may relax to half in the narrowest zones); wall-ring stations, grid holes, and nudges all cede narrow zones (local chord < 2·clearance + 1.6·pitch) to the centerline so thin strokes read as one clean centered row. AI/PDF import is a best-effort stdlib content-stream reader (m/l/c/v/y/re/h, q/Q/cm, clip paths discarded); EPS rejected with guidance. Output doc: outlines on `cut`, circles on `holes` — PDF preview + DXF download reuse existing plumbing. 43 tests in `tests/test_shape_infill.py`.
