...
- **Shape infill (2026-09):** `shape_infill.py` + "Infill a shape with perforations" UI section. Upload **DXF / SVG / AI-PDF** outlines (letters, logos, multi-graphic files, interior voids via even-odd) and fill with the straight/staggered hole grid (`_hole_centers` math, same margin semantics). Pure numpy containment (no shapely/GEOS): a hole is kept when its center is inside and ≥ `margin + hole_r` from every boundary segment. **Elastic lattice** (default; `spacing_flex` > 0, UI "Spacing flex %") is the main fill engine: ONE lattice with the exact stagger angle, globally swept over uniform scale (±flex) and phase for best fit, then locally relaxed (boundary-normal projection + crowding repulsion, each hole tethered ≤ flex×pitch to its node) so edge holes hug the outline at exact clearance while the interior stays on-lattice — even fill with no ring/grid seams; on friendly shapes it degenerates to the rigid grid (0 moved). Bare sharp corners get an anchor hole post-pass. **Perimeter outline row** (now default OFF — best for wide-stroke graphics; caused a "racetrack" look on letter-width strokes) traces every outline with corner-anchored holes at exact clearance, spacing justified per edge; fill holes crowding the ring (< 0.8×pitch) are excluded. Classic path (`spacing_flex=0`): **auto-align grid** phase sweep + **edge nudge** (capped inward slide, same Ø, min 1.05×Ø separation). **Stroke-width intelligence:** `analyze_strokes` measures stroke widths by chord-casting from the boundary and reports rows-across plus the pitch (or artwork scale, or a smaller hole Ø at pitch 2×Ø via `fit_dia`) needed for 3 rows; **auto-fit pitch** (opt-in) tightens the entered pitch to that value (floor `MIN_PITCH_FACTOR`=1.2×Ø). **Fill narrow strokes** (default on) chains a centerline row through zones too narrow for grid rows between the perimeter rings (edge margin may relax to half in the narrowest zones); wall-ring stations, grid holes, and nudges all cede narrow zones (local chord < 2·clearance + 1.6·pitch) to the centerline so thin strokes read as one clean centered row. AI/PDF import is a best-effort stdlib content-stream reader (m/l/c/v/y/re/h, q/Q/cm, clip paths discarded); EPS rejected with guidance. Output doc: outlines on `cut`, circles on `holes` — PDF preview + DXF download reuse existing plumbing. 38 tests in `tests/test_shape_infill.py`.
