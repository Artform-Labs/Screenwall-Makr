...
- **Shape infill (2026-09):** `shape_infill.py` + "Infill a shape with perforations" UI section. Upload **DXF / SVG / AI-PDF** outlines (letters, logos, multi-graphic files, interior voids via even-odd) and fill with the straight/staggered hole grid (`_hole_centers` math, same margin semantics). Pure numpy containment (no shapely/GEOS): a hole is kept when its center is inside and ≥ `margin + hole_r` from every boundary segment. **Perimeter outline row** (default on) traces every outline with corner-anchored holes at exact clearance, spacing justified per edge (corner-hole footprints trimmed so every gap on a side reads uniform); interior grid holes crowding the ring (< 0.8×pitch) are excluded. **Auto-align grid** sweeps the fill-grid phase (17 candidates) and keeps the alignment fitting the most holes — spacing untouched. **Edge nudge** slides an almost-fitting boundary hole up to a capped distance inward (same Ø, min 1.05×Ø center spacing to neighbors). **Stroke-width intelligence:** `analyze_strokes` measures stroke widths by chord-casting from the boundary and reports rows-across plus the pitch (or artwork scale) needed for 3 rows; **auto-fit pitch** (opt-in) tightens the entered pitch to that value (floor `MIN_PITCH_FACTOR`=1.2×Ø). **Fill narrow strokes** (default on) chains a centerline row through zones too narrow for grid rows between the perimeter rings (edge margin may relax to half in the narrowest zones); wall-ring stations, grid holes, and nudges all cede narrow zones (local chord < 2·clearance + 1.6·pitch) to the centerline so thin strokes read as one clean centered row. AI/PDF import is a best-effort stdlib content-stream reader (m/l/c/v/y/re/h, q/Q/cm, clip paths discarded); EPS rejected with guidance. Output doc: outlines on `cut`, circles on `holes` — PDF preview + DXF download reuse existing plumbing. 33 tests in `tests/test_shape_infill.py`.
